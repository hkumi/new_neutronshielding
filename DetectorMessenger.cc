...
{ 
  ...
  fMaterCmd->SetGuidance("Select material of the detector.");
  fMaterCmd->SetParameterName("choice",false);
  fMaterCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
  
  //select thickness
  fSizeCmd = new G4UIcmdWithADoubleAndUnit("/testhadr/det/setSize",this);
  fSizeCmd->SetGuidance("Set size of the detector (thickness)");
  ...
  //select position
  //fPosCmd = new G4UIcmdWith3VectorAndUnit("/testhadr/det/setPos",this);
  //fPosCmd->SetGuidance("Set position of the detector");
  //fPosCmd->SetParameterName("PosX","PosY","PosZ",false);
  //fPosCmd->SetRange("PosX>-50 && PosX<50 && PosY=0 && PosZ=0");
  //fPosCmd->SetDefaultUnit("cm");
  //fPosCmd->SetUnitCandidates("mm cm m");
  //fPosCmd->AvailableForStates(G4State_PreInit,G4State_Idle); 
...
{ 
  ...
  //if( command == fPosCmd )
  // { fDetector->SetPosition(fPosCmd->GetNew3VectorValue(newValue));}   
